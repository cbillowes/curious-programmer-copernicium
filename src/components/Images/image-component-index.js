/*
 * THIS FILE IS AUTO GENERATED.
 * It reads all images from ./src/images/articles
 *
 * Because React gave me the middle finger when dynamically importing/requiring
 * components, I have to generate a component index which I can then dynamically
 * access in my other components. o_O
 * https://stackoverflow.com/questions/48268507/react-dynamically-import-components
 **/

let Component = {};

Component['BlurredCycling'] = (props) =>
  require('./blurred-cycling').default(props);

Component['BusyStation'] = (props) => require('./busy-station').default(props);

Component['ChromeDevtoolsEmulation'] = (props) =>
  require('./chrome-devtools-emulation').default(props);

Component['Cisco'] = (props) => require('./cisco').default(props);

Component['ClojureFirebase'] = (props) =>
  require('./clojure-firebase').default(props);

Component['ClojureTailwindcss'] = (props) =>
  require('./clojure-tailwindcss').default(props);

Component['Clojure'] = (props) => require('./clojure').default(props);

Component['CogsWithGlobe'] = (props) =>
  require('./cogs-with-globe').default(props);

Component['CoupleKissingInTheDark'] = (props) =>
  require('./couple-kissing-in-the-dark').default(props);

Component['Curious'] = (props) => require('./curious').default(props);

Component['Curiousprogrammer.dev'] = (props) =>
  require('./curiousprogrammer.dev').default(props);

Component['Default01'] = (props) => require('./default-01').default(props);

Component['Default02'] = (props) => require('./default-02').default(props);

Component['Default03'] = (props) => require('./default-03').default(props);

Component['Default04'] = (props) => require('./default-04').default(props);

Component['Default05'] = (props) => require('./default-05').default(props);

Component['Default06'] = (props) => require('./default-06').default(props);

Component['Default07'] = (props) => require('./default-07').default(props);

Component['Default08'] = (props) => require('./default-08').default(props);

Component['Default09'] = (props) => require('./default-09').default(props);

Component['Default10'] = (props) => require('./default-10').default(props);

Component['DevRant'] = (props) => require('./dev-rant').default(props);

Component['DeviceEmulationPreview'] = (props) =>
  require('./device-emulation-preview').default(props);

Component['DogInBlanket'] = (props) =>
  require('./dog-in-blanket').default(props);

Component['Elastic'] = (props) => require('./elastic').default(props);

Component['Gatsby'] = (props) => require('./gatsby').default(props);

Component['Git'] = (props) => require('./git').default(props);

Component['HighriseBuildings'] = (props) =>
  require('./highrise-buildings').default(props);

Component['Kibana'] = (props) => require('./kibana').default(props);

Component['Mac'] = (props) => require('./mac').default(props);

Component['Mauritius'] = (props) => require('./mauritius').default(props);

Component['PagespeedScore'] = (props) =>
  require('./pagespeed-score').default(props);

Component['PensiveWomanWithRain'] = (props) =>
  require('./pensive-woman-with-rain').default(props);

Component['PeopleAtACafeteria'] = (props) =>
  require('./people-at-a-cafeteria').default(props);

Component['PinkMac'] = (props) => require('./pink-mac').default(props);

Component['PrettyGitGraph'] = (props) =>
  require('./pretty-git-graph').default(props);

Component['TwoPeoplePlanningAtATable'] = (props) =>
  require('./two-people-planning-at-a-table').default(props);

export default Component;
