...
date: 2022-11-03
modified: 2022-11-05
abstract: You will get create and populate your own instance of the MongoDB database.
---

## Objectives

1. Install and run your own copy of Mongo DB community.
1. Populate the database with the in-memory data you were using.
1. Learn how to read and write to Mongo DB documents.

## Install

[Install][install-community] and run the Mongo DB community service (version 6.0).
...
You can get all articles using the following comment.
The results will print formatted JSON when you append the command with the `pretty()` function.
...
Install the [npm][install-mongodb] package.
...
> **Note that** we will deal with hardcoded values later on.

```bash:title=>./
touch ./server/src/db.js
```
...
Now that you have a database, you can create functional endpoints.
